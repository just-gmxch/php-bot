...
echo "\n\033[1;35m馃敟 COOLFAUCET CREATOR BY - AKIEFX 馃嚥馃嚲\033[0m\n";
...
function claimCard($statusUrl, $claimUrl, $cookie, $ua, $ref)
{
    ...
    if (strpos($html, "Inventory Card") !== false) {
        echo "[" . date("H:i:s") . "] 馃儚 Kartu ditemukan! Mengklaim...\n";
...
function autoWithdraw($url, $cookie, $ua, $ref)
{
    ...
    echo "[" . date("H:i:s") . "] 馃捀 Auto WD dihantar\n";
}
...
function claimFaucet($cookie, $ua, $ref)
{
    ...
    if (strpos($html, "Faucet not ready yet") !== false) {
        echo "[" .
            date("H:i:s") .
            "] 鈴� Faucet belum sedia. Tunggu sebentar...\n";
        return;
    }
    $color = readCaptchaInstruction($html);
    if ($color) {
        echo "[" . date("H:i:s") . "] 馃 Arahan captcha: klik warna '$color'\n";
        $res = curlPost(
        ...
        echo "[" .
            date("H:i:s") .
            "] 鉁� Faucet diklaim dengan warna '$color'\n";
        echo "[" . date("H:i:s") . "] 馃摡 Respons: " . strip_tags($res) . "\n";
    } else {
        echo "[" . date("H:i:s") . "] 鈿狅笍 Arahan captcha tidak dijumpai\n";
    }
}
...
{
    while ($sec > 0) {
        echo "\033[1;33m鈴� Menunggu: {$sec}s\r\033[0m";
        sleep(1);
        $sec--;
    }
    echo "\n";
...
$lastFaucet = time();
while (true) {
    echo "\n[" . date("H:i:s") . "] 馃攧 Kitaran bermula...\n";

    $monster = getMonsterDetails($dashboard, $cookie, $userAgent, $referer);
    echo "[" . date("H:i:s") . "] 馃悏 Monster: {$monster["name"]}\n";
    echo "[" .
        date("H:i:s") .
        "] 鉂わ笍 HP: {$monster["hpNow"]} / {$monster["hpMax"]} | 馃 XP: {$monster["xp"]}\n";
    echo "[" .
        date("H:i:s") .
        "] 馃巵 Reward: {$monster["tokens"]} tokens ({$monster["ltc"]} LTC)\n";
    ...
    echo "[" . date("H:i:s") . "] 馃挵 Balance: \033[1;32m$balance LTC\033[0m\n";

    $dashboardHtml = curlGet($dashboard, $cookie, $userAgent, $referer);
    $tokenBalance = getTokenBalance($dashboardHtml);
    echo "[" . date("H:i:s") . "] 馃獧 Token Balance: $tokenBalance tokens\n";
